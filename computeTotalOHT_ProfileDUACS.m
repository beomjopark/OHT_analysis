...
%% Compute the in-situ OHT used for is2step
    ...
    if nTargetPres > 1
        target = load(['./Data/targetTempProf','ESA',tag,num2str(min(targetPres)),'_',num2str(max(targetPres)),dataYear,'.mat']);
...
%}
    ...
    targetTempProf = target.targetSSTProf;
...
%        drop = drop | isnan(profMeanRefVel);
    ...
    targetTempProf = targetTempProf(~drop);
%    targetDensProf = targetDensProf(targetPres==intStart, ~drop);
    ...
    %% Calculate flux
    %Convert Kelvin
%    targetTempProf = targetTempProf + 273.16; ALREADY KELVIN
    profVelAggr = profDerivSel ./ gsw_f(profLatAggrSel) .* gsw_grav(profLatAggrSel);
    if isAbsolute
...
%    profScaleInt = 1 ./ targetDensProf ./ gsw_grav(profLatAggrSel, intStart);
    ...
    profHeatFluxAggr = targetTempProf .* profVelAggr;
    profMassFluxAggr = profVelAggr;
...
%            profMassFluxAggrInt = profVelAggr; % Mass flux will be integrated from DerivSel
    ...
    if isAbsolute
        ...
        'profLatAggrSel','profLongAggrSel','profJulDayAggrSel',...
        'profHeatFluxAggr', 'profMassFluxAggr', 'profVelAggr',...
        'profHeatFluxAggrInt', 'profDerivSel', 'profVolFluxAggrInt',...
    ...
    if isPlot
        ...
        scatterm(profLatAggrSel,profLongAggrSel,10, targetSSTProf,'.');
