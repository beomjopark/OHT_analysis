function selectionAndIntegrate_Mid(dataYear, verticalSelection, intEnd, isPlot, iPart, nPart)
%% Reverse the intStart and intEnd under all pressure below reference
...
%% Note: Function version of selectionAndVerticalIntegrationPchipTrapzInterpolation_TempDens
    ...
    nIntEnd = numel(intEnd);
    if nIntEnd > 1
        fprintf("Multiple intEnd is provided.\n")
        isMultiple = true;
    else
        isMultiple = false;
    end
    ...
    % intEnd Should be provided if Relative Selection before the code runs
    if strcmp(verticalSelection, 'Relative') && (nargin == 2)
        error('Relative Profile requires intEnd input!');
    end
    ...
    switch verticalSelection
        ...
        case 'Relative'
            if isMultiple
                selIdx = (startPres >=0 & startPres <= intStart & endPres >= min(intEnd)); % Take the most lenient condition
            else
    ...
    if nargin > 4 % iPart exists
        ...
        nOriginal = length(endPres);
        nInterval = ceil(nOriginal / nPart);

        partIdx = false(size(endPres));
        if iPart == 1
...
%}
    ...
    endPres = endPres(selIdx); % for additional selection later.
...
%}
    ...
    targetDynhProf = NaN(nIntEnd, nProfile);

    % Check the top pressure bottle Idx for each profiles
    endPresIdx = sum( cumprod((endPres > intEnd'), 1), 1); %sum( cumprod((endPres <= intStart') == 0, 1), 1) + 1;

    tic;
    parfor iGrid = 1:nProfile
        endPresIgrid = endPresIdx(iGrid);
        intQuery = intEnd(1:endPresIgrid); %feasible bottles
        ...
        if numel(commonInt) == numel(intQuery) %all(profPresAggrSel{iGrid} == intQuery)
            profDynhAggrSel = gsw_geo_strf_dyn_height(profAbsSalAggrSel{iGrid}, profConsTempAggrSel{iGrid}, profPresAggrSel{iGrid}, intStart);
        else
...
%           targetIdx = (pres_i == intQuery);
           ...
           profDynhAggrSel = gsw_geo_strf_dyn_height(SA_i, CT_i, pres_i, intStart);
        end
    %{
    ...
    %}
        targetDynhProf(:, iGrid) = [profDynhAggrSel(targetIdx); NaN(nIntEnd - endPresIgrid, 1)];
    end
...
%   profDynhAggrSel{iGrid} = gsw_geo_strf_dyn_height(profAbsSalAggrSel{iGrid}, profConsTempAggrSel{iGrid}, profPresAggrSel{iGrid}, intEnd);
   toc;
    ...
    nGridx = 5000;
    gridx = linspace_vec(intStart.*ones(size(intEnd')), intEnd', nGridx); % Fine grid, takes about 13 mins to run

    nProfile = length(profPresAggrSel);
    intTempProf = NaN(nIntEnd, nProfile); % Potential Temperature
    intDensProf = NaN(nIntEnd, nProfile);
    ...
    parfor iGrid = 1:nProfile
        ...
        intTempIgrid = NaN(nIntEnd, 1);
        intDensIgrid = NaN(nIntEnd, 1);
        for idxIntEnd = 1:endPresIdx(iGrid)
            iGridx = gridx(idxIntEnd,:);
            ...
            intTempIgrid(idxIntEnd)  = trapz(iGridx, pchipIntPot);
            intDensIgrid(idxIntEnd)  = trapz(iGridx, pchipIntDens); 
        end
    ...
    if isMultiple
        intEndString = [num2str(min(intEnd)),'_',num2str(max(intEnd))];
    else
        intEndString = num2str(intEnd);
    end
    if nargin > 4 % part
        if strcmp(verticalSelection, 'Relative')
            saveName = ['./Data/intTempDensProf',tag,verticalSelection,intEndString,dataYear,'Part',num2str(iPart),'_',num2str(nPart),'.mat'];
        else
        ...
        save(saveName,...
            ...
            'intTempProf', 'intDensProf','targetDynhProf','intStart','intEnd', 'endPresIdx');
    else
        if strcmp(verticalSelection, 'Relative')
            saveName = ['./Data/intTempDensProf',tag,verticalSelection,intEndString,dataYear,'.mat'];
        else
